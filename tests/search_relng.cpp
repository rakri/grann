...
template<typename T>
int search_relng_index(int argc, char** argv) {
  T*                query = nullptr;
  ...
  std::string relng_file(argv[ctr++]);
...
int main(int argc, char** argv) {
  ...
  if (std::string(argv[1]) == std::string("int8"))
    search_relng_index<int8_t>(argc, argv);
  else if (std::string(argv[1]) == std::string("uint8"))
    search_relng_index<uint8_t>(argc, argv);
  else if (std::string(argv[1]) == std::string("float"))
    search_relng_index<float>(argc, argv);
  else
