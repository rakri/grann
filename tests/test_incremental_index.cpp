...
int main(int argc, char** argv) {
  ...
  grann::load_aligned_bin<float>(argv[1], data_load, num_points, dim,
                                   aligned_dim);
  ...
  typedef int                 TagT;
  grann::Index<float, TagT> index(grann::L2, argv[1], num_points,
  ...
  std::vector<grann::Neighbor>       pool, tmp;
  tsl::robin_set<unsigned>             visited;
  std::vector<grann::SimpleNeighbor> cut_graph;
