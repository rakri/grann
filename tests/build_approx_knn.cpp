...
template<typename T>
int build_approx_knn_index(const std::string&   data_path, const std::string &labels_file, 
  ...
  grann::ApproxKNN<T> aknn(metric, data_path.c_str(), idmap, labels_file);
  auto             s = std::chrono::high_resolution_clock::now();
  aknn.build(paras);
  std::chrono::duration<double> diff =
      std::chrono::high_resolution_clock::now() - s;

  aknn.save(save_path.c_str());
...
int main(int argc, char** argv) {
  ...
  if (std::string(argv[1]) == std::string("int8"))
    build_approx_knn_index<int8_t>(data_path, labels_file, metric, R, L, alpha, save_path,
                               num_threads);
  else if (std::string(argv[1]) == std::string("uint8"))
    build_approx_knn_index<uint8_t>(data_path, labels_file,  metric, R, L, alpha, save_path,
                                num_threads);
  else if (std::string(argv[1]) == std::string("float"))
    build_approx_knn_index<float>(data_path,  labels_file, metric, R, L, alpha, save_path,
                              num_threads);
  else
