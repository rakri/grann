    ...
    if (m == grann::Metric::FAST_L2) {
      ...
      if (Avx2SupportedCPU) {
        ...
        std::cout << "AVX2 not supported. Using AVX distance computation"
                  << std::endl;
        return new grann::AVXDistanceL2Float();
      } else {
        std::cout << "Older CPU. Using slow distance computation" << std::endl;
        return new grann::SlowDistanceL2Float();
      }
  ...
  grann::Distance<int8_t> *get_distance_function(grann::Metric m) {
    if (m == grann::Metric::L2) {
      if (Avx2SupportedCPU) {
        ...
        std::cout << "AVX2 not supported. Using AVX distance computation"
                  << std::endl;
        return new grann::AVXDistanceL2Int8();
      } else {
        std::cout << "Older CPU. Using slow distance computation" << std::endl;
        return new grann::SlowDistanceL2Int<int8_t>();
      }
    } else {
      std::stringstream stream;
      stream << "Only L2 metric supported as of now. Email "
...
namespace grann {
  ...
  Vamana<T, TagT>::Vamana(Metric m, const char *filename, const size_t max_points,
    ...
    if (_num_frozen_pts > 0) {
      ...
      if (_data == nullptr) {
        free(temp);
        grann::cout << "Realloc failed, killing programme" << std::endl;
        throw grann::ANNException("Realloc failed", -1, __FUNCSIG__, __FILE__,
                                    __LINE__);
      }
    }
        ...
        if (_num_points < _max_points) {
          ...
          for (unsigned i = 0; i < new_location.size(); i++)
            if ((_delete_set.find(i) == _delete_set.end()) &&
              ...
              grann::cout << "Wrong new_location assigned to  " << i
                            << std::endl;
            else {
              if ((_delete_set.find(i) != _delete_set.end()) &&
                  (new_location[i] < _max_points + _num_frozen_pts))
                grann::cout << "Wrong location assigned to "
                                 "delete point  "
                              << i << std::endl;
            }
          ...
          if (_enable_tags) {
            ...
            if (_can_delete) {
              std::cerr << "Disable deletes and consolidate "
                           "vamana before saving."
                        << std::endl;
              throw grann::ANNException(
                  "Disable deletes and consolidate vamana before "
                  "saving.",
    ...
    if (_lazy_done) {
      ...
      if (_enable_tags) {
        ...
        if (_can_delete || (!_consolidated_order)) {
          grann::cout << "Disable deletes and consolidate vamana before "
                           "saving."
                        << std::endl;
          throw grann::ANNException(
              "Disable deletes and consolidate vamana before saving.", -1,
  ...
  void Vamana<T, TagT>::load(const char *filename, const bool load_tags,
    ...
    if (load_tags) {
      if (_enable_tags == false)
        grann::cout << "Enabling tags." << std::endl;
      _enable_tags = true;
    ...
    while (cur_alpha <= alpha && result.size() < degree) {
      ...
      while (result.size() < degree && (start) < pool.size() && start < maxc) {
        ...
        for (unsigned t = start + 1; t < pool.size() && t < maxc; t++) {
          ...
          if (_metric == grann::Metric::L2) {
            occlude_factor[t] =
                (std::max)(occlude_factor[t], pool[t].distance / djk);
          } else if (_metric ==
                     grann::Metric::INNER_PRODUCT) {  // stylized rules for
                                                        // inner product since
                                                        // we want max instead
                                                        // of min distance
...
#pragma omp parallel for schedule(dynamic, 65536)
  ...
  std::pair<uint32_t, uint32_t> Vamana<T, TagT>::search(
    ...
    for (auto it : best_L_nodes) {
      ...
      if (_metric == grann::INNER_PRODUCT)
  ...
  int Vamana<T, TagT>::eager_delete(const TagT        tag,
    ...
    unsigned              Lvamana = parameters.Get<unsigned>("L");
    std::vector<unsigned> init_ids;

    get_expanded_nodes(id, Lvamana, init_ids, pool, visited);
  ...
  void Vamana<T, TagT>::compact_data(std::vector<unsigned> new_location,
    ...
    grann::cout << "Re-numbering nodes and edges and consolidating data... "
                  << std::flush;
    grann::cout << "active = " << active << std::endl;
    for (unsigned old = 0; old < _max_points + _num_frozen_pts; ++old) {
      if (new_location[old] <
    ...
    grann::cout << "Updating mapping between tags and ids... " << std::flush;
  ...
  void Vamana<T, TagT>::readjust_data(unsigned _num_frozen_pts) {
    if (_num_frozen_pts > 0) {
      ...
      grann::cout << "No frozen points. No re-adjustment required"
                    << std::endl;
  }
  ...
  int Vamana<T, TagT>::insert_point(const T *point, const Parameters &parameters,
    ...
    std::vector<unsigned> init_ids;
    get_expanded_nodes(location, Lvamana, init_ids, pool, visited);
  ...
  int Vamana<T, TagT>::disable_delete(const Parameters &parameters,
    ...
    LockGuard guard(_change_lock);
    if (!_can_delete) {
      grann::cerr << "Delete not currently enabled" << std::endl;
      return -1;
    }
    if (!_enable_tags) {
      grann::cerr << "Point tag array not instantiated" << std::endl;
      throw grann::ANNException("Point tag array not instantiated", -1,
                                  __FUNCSIG__, __FILE__, __LINE__);
    }
    if (_eager_done) {
    ...
    if (consolidate) {
      ...
      if (nd >= 0)
        grann::cout << "#Points after consolidation: " << nd + _num_frozen_pts
                      << std::endl;
    }
  ...
  int Vamana<T, TagT>::delete_point(const TagT tag) {
    ...
    LockGuard guard(_change_lock);
    if (_tag_to_location.find(tag) == _tag_to_location.end()) {
      grann::cerr << "Delete tag not found" << std::endl;
      return -1;
    }
