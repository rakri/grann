...
namespace grann {
  ...
  template<typename T>
    ...
    grann::save_bin<float>(centers_file, _cluster_centers, _num_clusters, this->_aligned_dim);
    ...
    grann::cout<<"Written a total of " << total_count << " points to inverted index file." << std::endl;
    }
  ...
  template<typename T>
    ...
    grann::load_aligned_bin<float>(centers_file, _cluster_centers, _num_clusters, tmp_dim1, tmp_dim2);
...
_inverted_index.resize(_num_clusters);
    ...
    in.close();
    grann::cout<<"Read a total of " << total_count << " points from inverted index file." << std::endl;
...
_u64 num_train;
  ...
  gen_random_slice<T>(this->_data, this->_num_points, this->_aligned_dim, training_prob, train_data_float, num_train);
grann::cout<<"Going to train the cluster centers over " << num_train <<" points " << std::endl;

math_utils::kmeans_plus_plus_centers(train_data_float, num_train, this->_aligned_dim,
                                       _cluster_centers, _num_clusters);

math_utils::run_lloyds(train_data_float, num_train, this->_aligned_dim, _cluster_centers,
                         _num_clusters, MAX_K_MEANS_REPS, nullptr, nullptr);
...
grann::convert_types(this->_data, data_float, this->_num_points, this->_aligned_dim);
math_utils::compute_closest_centers(data_float, this->_num_points, this->_aligned_dim,
                               _cluster_centers, _num_clusters, 1,
                               closest_centers.data(),
                               _inverted_index.data(),
                               nullptr);
