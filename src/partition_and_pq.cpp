...
int generate_pq_pivots(const float *passed_train_data, size_t num_train,
  ...
  if (num_pq_chunks > dim) {
    grann::cout << " Error: number of chunks more than dimension"
                  << std::endl;
    return -1;
  }
  ...
  if (file_exists(pq_pivots_path)) {
    ...
    if (file_dim == dim && file_num_centers == num_centers) {
      grann::cout << "PQ pivot file exists. Not generating again"
                    << std::endl;
      return -1;
    }
  }
  ...
  for (uint32_t d = 0; d < dim; d++) {
    ...
    grann::cout << " Pushing " << d << " into bin #: " << cur_best
                  << std::endl;
...
#pragma omp parallel for schedule(static, 65536)
  ...
  grann::save_bin<uint32_t>(chunk_offsets_path.c_str(), chunk_offsets.data(),
...
int generate_pq_data_from_pivots(const std::string data_file,
  ...
  if (!file_exists(pq_pivots_path)) {
    ...
    if (numr != dim || numc != 1) {
      ...
      throw grann::ANNException("Error reading centroid file.", -1,
...
#pragma omp parallel for schedule(static, 8192)
    ...
    if (num_centers > 256) {
      ...
      grann::convert_types<uint32_t, uint8_t>(
...
int shard_data_into_clusters(const std::string data_file, float *pivots,
  ...
  for (size_t block = 0; block < num_blocks; block++) {
    ...
    base_reader.read((char *) block_data_T.get(),
                     sizeof(T) * (cur_blk_size * dim));
    grann::convert_types<T, float>(block_data_T.get(), block_data_float.get(),
                                     cur_blk_size, dim);
...
int retrieve_shard_data_from_ids(const std::string data_file,
  ...
  _u32 *shard_ids;
  _u64  shard_size, tmp;
  grann::load_bin<_u32>(idmap_filename, shard_ids, shard_size, tmp);
  ...
  grann::cout << "Written file with " << num_written << " points"
                << std::endl;
  ...
  while (!fit_in_ram) {
    ...
    for (auto &p : cluster_sizes) {
      ...
      double cur_shard_ram_estimate =
          ESTIMATE_VAMANA_RAM_USAGE(p, train_dim, sizeof(T), graph_degree);
...
template void GRANN_DLLEXPORT
              gen_random_slice<int8_t>(const std::string base_file,
                         const std::string output_prefix, double sampling_rate);
template void GRANN_DLLEXPORT gen_random_slice<uint8_t>(
...
template void GRANN_DLLEXPORT gen_random_slice<int8_t>(
    const int8_t *inputdata, size_t npts, size_t ndims, double p_val,
    float *&sampled_data, size_t &slice_size);

template void GRANN_DLLEXPORT gen_random_slice<float>(
...
template GRANN_DLLEXPORT int partition<int8_t>(
...
template GRANN_DLLEXPORT int partition<uint8_t>(
    const std::string data_file, const float sampling_rate, size_t num_centers,
    size_t max_k_means_reps, const std::string prefix_path, size_t k_base);
template GRANN_DLLEXPORT int partition<float>(
    const std::string data_file, const float sampling_rate, size_t num_centers,
    size_t max_k_means_reps, const std::string prefix_path, size_t k_base);

template GRANN_DLLEXPORT int partition_with_ram_budget<int8_t>(
    const std::string data_file, const double sampling_rate, double ram_budget,
    size_t graph_degree, const std::string prefix_path, size_t k_base);
template GRANN_DLLEXPORT int partition_with_ram_budget<uint8_t>(
    const std::string data_file, const double sampling_rate, double ram_budget,
    size_t graph_degree, const std::string prefix_path, size_t k_base);
template GRANN_DLLEXPORT int partition_with_ram_budget<float>(
    const std::string data_file, const double sampling_rate, double ram_budget,
    size_t graph_degree, const std::string prefix_path, size_t k_base);

template GRANN_DLLEXPORT int retrieve_shard_data_from_ids<float>(
    const std::string data_file, std::string idmap_filename,
    std::string data_filename);
template GRANN_DLLEXPORT int retrieve_shard_data_from_ids<uint8_t>(
    const std::string data_file, std::string idmap_filename,
    std::string data_filename);
template GRANN_DLLEXPORT int retrieve_shard_data_from_ids<int8_t>(
    const std::string data_file, std::string idmap_filename,
    std::string data_filename);

template GRANN_DLLEXPORT int generate_pq_data_from_pivots<int8_t>(
...
template GRANN_DLLEXPORT int generate_pq_data_from_pivots<uint8_t>(
    const std::string data_file, unsigned num_centers, unsigned num_pq_chunks,
    std::string pq_pivots_path, std::string pq_compressed_vectors_path);
template GRANN_DLLEXPORT int generate_pq_data_from_pivots<float>(
    const std::string data_file, unsigned num_centers, unsigned num_pq_chunks,
    std::string pq_pivots_path, std::string pq_compressed_vectors_path);
