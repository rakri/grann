...
namespace grann {
  ...
  PQFlashIndex<_u8>::PQFlashIndex(
    ...
    if (Avx2SupportedCPU) {
      grann::cout << "Using AVX2 dist_cmp_float function." << std::endl;
      this->dist_cmp_float = new DistanceL2();
    } else if (AvxSupportedCPU) {
      grann::cout << "Using AVX dist_cmp_float function" << std::endl;
      this->dist_cmp_float = new AVXDistanceL2Float();
    } else {
      grann::cout << "No AVX/AVX2 support. Using Slow dist_cmp_float function"
                    << std::endl;
  ...
  PQFlashIndex<_s8>::PQFlashIndex(
      ...
      : reader(fileReader), metric(metric) {
    if (Avx2SupportedCPU) {
      grann::cout << "Using AVX2 function for dist_cmp and dist_cmp_float"
                    << std::endl;
      this->dist_cmp = new DistanceL2Int8();
      this->dist_cmp_float = new DistanceL2();
    } else if (AvxSupportedCPU) {
      grann::cout << "No AVX2 support. Switching to AVX routines for "
                       "dist_cmp, dist_cmp_float."
                    << std::endl;
      this->dist_cmp = new AVXDistanceL2Int8();
      this->dist_cmp_float = new AVXDistanceL2Float();
    } else {
      grann::cout << "No AVX/AVX2 support. Switching to slow routines for "
                       "dist_cmp, dist_cmp_float"
                    << std::endl;
  ...
  PQFlashIndex<float>::PQFlashIndex(
    ...
    if (metric == grann::Metric::L2) {
      if (Avx2SupportedCPU) {
        grann::cout << "Using AVX2 functions for dist_cmp and dist_cmp_float"
                      << std::endl;
        this->dist_cmp = new DistanceL2();
        this->dist_cmp_float = new DistanceL2();
      } else if (AvxSupportedCPU) {
        grann::cout << "No AVX2 support. Switching to AVX functions for "
    ...
    } else if (metric == grann::Metric::INNER_PRODUCT) {
  ...
  PQFlashIndex<T>::~PQFlashIndex() {
    ...
    if (nhood_cache_buf != nullptr) {
      ...
      grann::aligned_free(coord_cache_buf);
    }
...
#pragma omp critical
      {
        ...
        grann::alloc_aligned((void **) &scratch.coord_scratch,
                               coord_alloc_size, 256);
        // scratch.coord_scratch = new T[MAX_N_CMPS * this->aligned_dim];
        // //Gopal. Commenting out the reallocation!
        grann::alloc_aligned((void **) &scratch.sector_scratch,
                               MAX_N_SECTOR_READS * SECTOR_LEN, SECTOR_LEN);
        grann::alloc_aligned((void **) &scratch.aligned_scratch,
                               256 * sizeof(float), 256);
        grann::alloc_aligned((void **) &scratch.aligned_pq_coord_scratch,
                               25600 * sizeof(_u8), 256);
        grann::alloc_aligned((void **) &scratch.aligned_pqtable_dist_scratch,
                               25600 * sizeof(float), 256);
        grann::alloc_aligned((void **) &scratch.aligned_dist_scratch,
                               512 * sizeof(float), 256);
        grann::alloc_aligned((void **) &scratch.aligned_query_T,
                               this->aligned_dim * sizeof(T), 8 * sizeof(T));
        grann::alloc_aligned((void **) &scratch.aligned_query_float,
  ...
  void PQFlashIndex<T>::destroy_thread_data() {
    ...
    while (this->thread_data.size() > 0) {
      ...
      grann::aligned_free((void *) scratch.coord_scratch);
      grann::aligned_free((void *) scratch.sector_scratch);
      grann::aligned_free((void *) scratch.aligned_scratch);
      grann::aligned_free((void *) scratch.aligned_pq_coord_scratch);
      grann::aligned_free((void *) scratch.aligned_pqtable_dist_scratch);
      grann::aligned_free((void *) scratch.aligned_dist_scratch);
      grann::aligned_free((void *) scratch.aligned_query_float);
      grann::aligned_free((void *) scratch.aligned_query_T);
    }
  }
  ...
  void PQFlashIndex<T>::load_cache_list(std::vector<uint32_t> &node_list) {
    ...
    grann::alloc_aligned((void **) &coord_cache_buf,
  ...
  void PQFlashIndex<T>::generate_cache_list_from_sample_queries(
    ...
    if (files.fileExists(sample_bin)) {
      grann::load_aligned_bin<T>(files, sample_bin, samples, sample_num,
                                   sample_dim, sample_aligned_dim);
    }
#else
    if (file_exists(sample_bin)) {
      grann::load_aligned_bin<T>(sample_bin, samples, sample_num, sample_dim,
                                   sample_aligned_dim);
    }
#endif
    else {
      grann::cerr << "Sample bin file not found. Not generating cache."
                    << std::endl;
      return;
    }
...
#pragma omp parallel for schedule(dynamic, 1) num_threads(nthreads)
  ...
  void PQFlashIndex<T>::cache_bfs_levels(_u64 num_nodes_to_cache,
    ...
    while ((node_list.size() + cur_level->size() < num_nodes_to_cache) &&
      ...
      grann::cout << "Level: " << lvl << std::flush;
      ...
      grann::cout << ". #nodes: " << node_list.size() - prev_node_list_size
  ...
  void PQFlashIndex<T>::use_medoids_data_as_centroids() {
    ...
    grann::cout << "Loading centroid data from medoids vector data of "
  ...
  int PQFlashIndex<T>::load(uint32_t num_threads, const char *pq_prefix,
    ...
    std::string medoids_file = std::string(disk_vamana_file) + "_medoids.bin";
    std::string centroids_file =
        std::string(disk_vamana_file) + "_centroids.bin";
    ...
    this->disk_vamana_file = std::string(disk_vamana_file);

    if (pq_file_num_centroids != 256) {
      grann::cout << "Error. Number of PQ centroids is not 256. Exitting."
                    << std::endl;
      return -1;
    }
...
#ifdef EXEC_ENV_OLS
    grann::load_bin<_u8>(files, pq_compressed_vectors, this->data, npts_u64,
                           nchunks_u64);
#else
    grann::load_bin<_u8>(pq_compressed_vectors, this->data, npts_u64,
                           nchunks_u64);
#endif
    ...
    std::string disk_pq_pivots_path = this->disk_vamana_file + "_pq_pivots.bin";
    if (file_exists(disk_pq_pivots_path)) {
      use_disk_vamana_pq = true;
#ifdef EXEC_ENV_OLS
      ...
      std::cout << "Disk vamana uses PQ data compressed down to "
                << disk_pq_n_chunks << " bytes per point." << std::endl;
    }

// read vamana metadata
#ifdef EXEC_ENV_OLS
    // This is a bit tricky. We have to read the header from the
    // disk_vamana_file. But  this is
    ...
    reader->open(disk_vamana_file);
    this->setup_thread_data(num_threads);
    this->max_nthreads = num_threads;

    char *                   bytes = getHeaderBytes();
    ContentBuf               buf(bytes, HEADER_SIZE);
    std::basic_istream<char> vamana_metadata(&buf);
#else
    std::ifstream vamana_metadata(disk_vamana_file, std::ios::binary);
#endif

    size_t actual_vamana_size = get_file_size(disk_vamana_file);
    size_t expected_file_size;
    READ_U64(vamana_metadata, expected_file_size);
    if (actual_vamana_size != expected_file_size) {
      grann::cout << "File size mismatch for " << disk_vamana_file
                    << " (size: " << actual_vamana_size << ")"
    ...
    _u64 disk_nnodes;
    READ_U64(vamana_metadata, disk_nnodes);
    if (disk_nnodes != num_points) {
      grann::cout << "Mismatch in #points for compressed data file and disk "
                       "vamana file: "
    ...
    READ_U64(vamana_metadata, medoid_id_on_file);
    READ_U64(vamana_metadata, max_node_len);
    ...
    delete[] bytes;
#else
    vamana_metadata.close();
#endif

#ifndef EXEC_ENV_OLS
    // open AlignedFileReader handle to vamana_file
    std::string vamana_fname(disk_vamana_file);
    reader->open(vamana_fname);
    this->setup_thread_data(num_threads);
    this->max_nthreads = num_threads;
    ...
    if (files.fileExists(medoids_file)) {
      size_t tmp_dim;
      grann::load_bin<uint32_t>(files, medoids_file, medoids, num_medoids,
                                  tmp_dim);
#else
    if (file_exists(medoids_file)) {
      size_t tmp_dim;
      grann::load_bin<uint32_t>(medoids_file, medoids, num_medoids, tmp_dim);
#endif
...
#ifdef EXEC_ENV_OLS
        grann::load_aligned_bin<float>(files, centroids_file, centroid_data,
                                         num_centroids, tmp_dim,
                                         aligned_tmp_dim);
#else
        grann::load_aligned_bin<float>(centroids_file, centroid_data,
                                         num_centroids, tmp_dim,
                                         aligned_tmp_dim);
#endif
    ...
    std::string norm_file = std::string(disk_vamana_file) + "_max_base_norm.bin";

    if (file_exists(norm_file) && metric == grann::Metric::INNER_PRODUCT) {
      _u64   dumr, dumc;
      float *norm_val;
      grann::load_bin<float>(norm_file, norm_val, dumr, dumc);
...
//std::cout<<l_search<<" ";
    ...
    if (metric == grann::Metric::INNER_PRODUCT) {
    ...
    */
    ...
    for (_u64 i = 0; i < k_search; i++) {
      ...
      if (distances != nullptr) {
        ...
        if (metric == grann::Metric::INNER_PRODUCT) {  // flip the sign from
                                                         // convert min to max
